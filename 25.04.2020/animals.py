...
class Animal(pygame.sprite.Sprite):
    ...
    def lookForFood(self, dist, window_rect):
        food = dist
        if food.x > self.speed:
            directionX = (food.x - self.x)
            if directionX>0:
            ...
            # x < speed:
            self.rect.x = food.x
        if food.y > self.speed:
            directionY = (food.y - self.y)
            if directionY>0:
...
class Peacock(Animal):
    ...
    def detectFood(self,ListP, ListT):
        ...
        nearestFood=min([food for food in foodList], key=lambda food: pos.distance_to(pygame.math.Vector2(food.x, food.y)))
        return nearestFood
...
class Tiger(Animal):
    def __init__(self):
        ...
        #self.foodList = config.tigers['foodList']
